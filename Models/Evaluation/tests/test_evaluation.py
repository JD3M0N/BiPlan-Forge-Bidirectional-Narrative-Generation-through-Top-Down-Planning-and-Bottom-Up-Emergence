...
def test_template_and_migration_are_idempotent(tmp_path):
    ...
    assert migrate_story_evaluations(tmp_path) == [first]
...
def test_failed_replace_preserves_existing_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("asg_evaluation.evaluation.os.replace", lambda *_: (_ for _ in ()).throw(OSError("boom")))
