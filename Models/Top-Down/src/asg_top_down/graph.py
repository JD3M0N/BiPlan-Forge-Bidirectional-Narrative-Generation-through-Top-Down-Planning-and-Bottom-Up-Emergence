...
def materialize_plan(
    ...
    chapters = sorted(draft.chapters, key=lambda item: item.order)
...
def validate_story_plan(
    ...
    chapter_order_by_id = {item.id: item.order for item in plan.chapters}
    ...
    events_by_chapter = {identifier: 0 for identifier in chapter_ids}
    ...
    ordered_events = sorted(plan.events, key=lambda item: item.order)
