...
from asg_top_down.generator import _parse_chapter_bodies
...
def make_characters() -> CharactersArtifact:
    return CharactersArtifact(characters=[CharacterProfile(
        id="ana", name="Ana", role="protagonist", goal="Reveal the truth",
        motivation="Protect her sister", conflict="Revelation risks her home",
        arc="Learns to trust others", voice="Precise and restrained",
    )])
...
def valid_plan() -> StoryPlanDraft:
    return StoryPlanDraft(
        ...
        dependencies=[
            ...
            EventDependency(
                source_event_id="event-2", target_event_id="event-3", relation="causal",
            ),
        ],
    )
...
def test_complete_pipeline_saves_small_v5_artifact_set(tmp_path) -> None:
    ...
    expected = {
        "request.json", "world.json", "characters.json", "story_plan.json",
        "draft.md", "review.json", "length_audit.json", "story.md",
        "metadata.json", "pipeline_manifest.json", "llm_usage.json",
    }
...
def test_invalid_plan_is_replaced_once(tmp_path) -> None:
    ...
    second_prompt = [prompt for name, prompt in provider.structured_calls if name == "StoryPlanDraft"][1]
...
def test_late_review_failure_delivers_the_draft_with_warning(tmp_path) -> None:
    ...
    assert run.story_path.read_text(encoding="utf-8") == (
        run.run_dir / "draft.md"
...
def test_final_chapter_parser_requires_every_heading() -> None:
    story = "# Título\n\n## Uno\n\nPrimero.\n\n## Dos\n\nSegundo."
    assert _parse_chapter_bodies(story, 2) == ["Primero.", "Segundo."]
    assert _parse_chapter_bodies(story, 3) == []
