...
class PlotPlannerAgent(Agent[StoryPlanDraft]):
    ...
    def run(
        ...
        aim_events = profile_event_aim(request.narrative_profile)
        return self.provider.generate_structured(
            system_instruction=(
                ...
                f"{high_events} events in total for that chapter range. Aim at {aim_events} "
                f"events; {low_events} is the rejection boundary, not the goal, and a plan below "
                "it or with a chapter carrying a single event is sent back. Never let one chapter "
                "absorb most of the story. "
                ...
                "not a branch. Worked example of a valid branch and join, shown as a fragment "
                "of a larger plan whose IDs are illustrative and imply no event count: "
                "event_2 -> event_4 and event_2 -> event_5 make event_2 the branch, then "
                "event_4 -> event_6 and event_5 -> event_6 make event_6 the join; every edge "
                "points forward (2 < 4, 2 < 5, 4 < 6, 5 < 6) and the branch comes before the "
                "join. "
