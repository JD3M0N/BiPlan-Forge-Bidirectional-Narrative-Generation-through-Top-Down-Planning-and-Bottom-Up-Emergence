"""Command-line interface for individual runs and reproducible experiments."""
...
def parser() -> argparse.ArgumentParser:
    ...
    default_map = root / "packages" / "escape_room" / "maps" / "escape_room.json"
...
def room_with_agents(path: Path, count: int):
    """Handle the room with agents operation for component."""
    room = load_room(path)
...
def run_one(args: argparse.Namespace) -> Path:
    ...
    try:
        ...
        result, model = run_simulation(room, seed=seed, tick_limit=args.tick_limit)
