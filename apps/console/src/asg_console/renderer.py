...
class ConsoleRenderer:
    """Represent ConsoleRenderer data and behavior."""
    ...
    def views(self, model: EscapeRoomModel) -> list[str]:
        """Handle the views operation for ConsoleRenderer."""
    ...
    def render(
        ...
        *,
        ...
        known_cells = selected.beliefs.known_cells if selected is not None else None
        lines = [
        ...
        if last_record is not None:
            ...
            for resolution in last_record.resolutions:
                ...
                lines.append(f"  {action.actor_id}: {action.kind.value}{target} [{status}]")
    ...
    def _symbol(self, model, position, selected) -> str:
        """Handle the symbol operation for ConsoleRenderer."""
        world = model.world
        ...
        for obj in world.objects.values():
            if obj.position == position:
                if selected is None or obj.id in selected.beliefs.known_objects:
                    return self.OBJECT_SYMBOLS.get(obj.id, "o")
        for feature in world.features.values():
            if feature.position == position:
                if selected is None or feature.id in selected.beliefs.known_features:
