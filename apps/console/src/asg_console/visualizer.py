"""Interactive loop for visual Bottom-Up simulations."""
...
class VisualOutcome:
    """Represent VisualOutcome data and behavior."""
...
class EscapeRoomVisualizer:
    """Represent EscapeRoomVisualizer data and behavior."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the EscapeRoomVisualizer instance."""
    ...
    def run(self, model: EscapeRoomModel, *, tick_limit: int = 300) -> VisualOutcome:
        """Run the EscapeRoomVisualizer workflow."""
        last_record: TickRecord | None = None
        next_tick = self.clock.monotonic()
        self._draw(model, last_record)
        try:
            with self.keyboard:
                while model.world.tick < tick_limit and not all(
                    agent.escaped for agent in model.world.characters.values()
                ):
                    step_once = False
                    ...
                    if key:
                        ...
                        if key == " ":
                            ...
                            self.view_index = (self.view_index + 1) % len(
                                self.renderer.views(model)
                            )
                        self._draw(model, last_record)
    ...
    def _draw(self, model: EscapeRoomModel, last_record: TickRecord | None) -> None:
        """Handle the draw operation for EscapeRoomVisualizer."""
